...
static int op_selected = 0; // 0=MatVec, 1=MatMul, 2=Solve(NoPivot), 3=Solve(PartialPivot)

// Dades
static Matrix A, B; // entrades (A: m x k), (B: k x n)
static Vec    x, b; // x: k, b: n_solve

// Resultats
static Vec    y;      // per MatVec
static Matrix C;      // per MatMul
...
int main()
{
    ...
    SDL_Window* window = SDL_CreateWindow(
        "Laboratori 1: - Algebra Lineal Numerica",
        1280, 720,
    ...
    while (running)
    {
        ...
        if (show_datasets)
        {
            if (ImGui::Begin("Datasets", &show_datasets))
            {
                ...
                if (op_selected <= 1) { // MatVec / MatMul
                    ImGui::InputInt("m (files A)", &m_rows); if (m_rows < 1) m_rows = 1;
                    ImGui::InputInt("kA (cols A)", &a_cols); if (a_cols < 1) a_cols = 1;
                    if (op_selected == 1) {
                        ImGui::InputInt("kB (files B)", &b_rows); if (b_rows < 1) b_rows = 1;
                        ImGui::InputInt("n (cols B)", &n_cols); if (n_cols < 1) n_cols = 1;
                        ImGui::BeginDisabled(true);
                        ImGui::InputInt("|x| (mida del vector)", &x_len); if (x_len < 1) x_len = 1;
                        ImGui::EndDisabled();
                    }
                    else {
                        ImGui::BeginDisabled(true);
                ...
                if (ImGui::Button("Genera aleatori"))
                {
                    ...
                    if (op_selected <= 1) {
                        ...
                        A = Matrix(m, k, 0.0);
                        B = (op_selected == 1) ? Matrix(b_rows, n, 0.0) : Matrix();
                        x = Vec((size_t)x_len);
                        // A: si no quadrada, identitat parcial + soroll
                        for (int i = 0; i < m; ++i) {
                            for (int j = 0; j < k; ++j) { A.At((size_t)i, (size_t)j) = U(rng) * 0.5; }
                            if (i < k) A.At((size_t)i, (size_t)i) += 1.0; // fer-la ben condicionada
                        }
                        if (op_selected == 1) {
                            for (int i = 0; i < b_rows; ++i) for (int j = 0; j < n; ++j) B.At((size_t)i, (size_t)j) = U(rng);
                        }
                        for (int j = 0; j < x_len; ++j) x[(size_t)j] = U(rng);
                        // b no s'usa en MatVec/MatMul
                    }
                    else {
                        int n = n_solve;
                        A = Matrix(n, n, 0.0); b = Vec((size_t)n);
                        for (int i = 0; i < n; ++i) {
                            double rowsum = 0.0; for (int j = 0; j < n; ++j) if (i != j) { double val = U(rng); A.At((size_t)i, (size_t)j) = val; rowsum += std::fabs(val); }
                            A.At((size_t)i, (size_t)i) = rowsum + 1.0; // diag. dominant per no singular
                            b[(size_t)i] = U(rng);
                        }
                    }
                    // neteja resultats
                ...
                bool small = false;
                if (op_selected <= 1)
                    small = (A.rows <= 8 && A.cols <= 8) && (op_selected == 0 || (B.rows <= 8 && B.cols <= 8));
                else
                ...
                if (op_selected < 1) DrawVecPreview("x", x, small);
                if (op_selected >= 2) DrawVecPreview("b", b, small);
        ...
        if (show_ops)
        {
            ...
            {
                const char* ops[] = { "MatVec (A*x)", "MatMul (A*B)", "Solve (No Pivot)", "Solve (Partial Pivot)" };
                ...
                if (ImGui::BeginPopupModal("error_popup", NULL, ImGuiWindowFlags_AlwaysAutoResize))
                {
                    ...
                    ImGui::Separator();
                    if (op_selected <= 1) ImGui::TextDisabled("Esperat: A %dx%d, B %dx%d (si MatMul), x %d", m_rows, a_cols, a_cols, n_cols, x_len);
                    else ImGui::TextDisabled("Esperat: A %dx%d, b %d", n_solve, n_solve, n_solve);
        ...
        if (show_results)
        {
            if (ImGui::Begin("Resultats", &show_results))
            {
                ...
                if (C.rows) DrawMatPreview("C = A*B", C, false);
